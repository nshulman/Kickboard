   ...
   @api refresh() {
    ...
    @api createMetadataBlueprintCard(event,isPrimary) {
        ...
        if(event.detail.isEdit){
            ...
            this.editRecordId='';
            personastepId = isPrimary?event.detail.id:event.target.dataset.id;
    ...
    createRecord(event){
        if(event.detail && event.detail.objectName){
            if(event.detail.objectName===this.blueprintPersonaStepApiName.objectApiName && event.detail.fields != null){
                this.createBlueprintPersona(event.detail.fields);
            }else if(event.detail.objectName === this.blueprintCardApiName.objectApiName && event.detail.fields !=null){
    ...
    dropPersona(event){
        ...
        if(relatedStepToDrag === relatedStepToDrop){
           ...
           this.performDragAndDropForPersona(dragId,dropId,relatedStepToDrag);
        }
    }
    ...
    createFormForNewPersona(event){
        ...
        let personastepId = this.currentStepId && this.currentStepId.length>0 ? this.currentStepId[currentIndex]:'';
        if (personastepId) {
            this.isModalOpen = true;
